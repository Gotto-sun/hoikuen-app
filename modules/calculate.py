...
EXCLUDED_INGREDIENT_PATTERN = re.compile(
    ...
    r"こしょう|胡椒|ソース|ケチャップ|マヨネーズ|中華だし|カレー粉|片栗粉|片栗|片困粉|きな粉|きなこ|黄粉|しょうゆせんべい"
)
...
STANDARD_NAME_RULES = [
    ...
    ("コーン缶", re.compile(r"コーン缶|コーン向|とうもろこし缶|トウモロコシ缶|コーン")),
    ...
    ("きゅうり", re.compile(r"きゅうり|きゆうり|きめゅうり|さきゅうり|胡瓜|きゅうの")),
    ("もやし", re.compile(r"もやし|よやし|(?:^|[^ぁ-んァ-ン一-龥])もや(?:$|[^ぁ-んァ-ン一-龥])")),
    ("木綿豆腐", re.compile(r"木綿豆腐|木綿とうふ|木綿豆富|豆放")),
    ("みかん缶", re.compile(r"みかん缶|みかん和缶|みかん人缶|ミカン缶|蜜柑缶|みかんかん")),
    ...
    ("ブロッコリー", re.compile(r"ブロッコリー|ブロツコリー|ブロコリー|ブロッコリ|プロッコリー|ブロシンコリ|ブロシッコリー")),
...
def _standard_name(name: object) -> str:
    ...
    if "鶏もも(皮なし)" in str(name or "") or "鶏もも皮なし" in original_compact or "鶏もも肉皮ない" in original_compact or "鶏モモ肉皮なし" in original_compact:
        return "鶏もも(皮なし)"
