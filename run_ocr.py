...
EXCLUDED_INGREDIENT_PATTERN = re.compile(r"スチコン|オーブン|コンビモード|レンジ|機器|器具|コンソメ|ョヨンツメ|ヨ肥外|米$|^米$|精白米|白米|ごはん|御飯|だし|出汁|だし汁|煮干しだし|かつおだし|昆布だし|水$|調味料|調味料全般|食塩|塩$|砂糖|しょうゆ$|醤油$|みそ|味噌|酢$|油$|サラダ油|ごま油|酒$|みりん|こしょう|胡椒|ソース|ケチャップ|マヨネーズ|中華だし|カレー粉|片栗粉|片栗|片困粉|きな粉|きなこ|黄粉|用本明|有本塊")
...
ROUNDING_ORDER_RULES = [
    ...
    ("鶏もも(皮なし)", "kg", 0.001, {"g": 1000.0}, re.compile(r"鶏もも(?:肉)?|鶏モモ(?:肉)?|鶏肉|とりもも肉")),
    ...
    ("ツナ油漬け缶", "缶", 1.0, {"缶": 1.0, "個": 1.0}, re.compile(r"ツナ")),
    ("缶詰", "缶", 1.0, {"缶": 1.0, "個": 1.0}, re.compile(r"缶詰|みかん缶|桃缶|パイン缶")),
    ("コーン缶", "缶", 1.0, {"缶": 1.0, "個": 1.0}, re.compile(r"コーン缶|コーン笛")),
]

PRIORITY_FOOD_PATTERN = re.compile(r"にんじん|人参|たまねぎ|玉ねぎ|玉葱|じゃがいも|馬鈴薯|キャベツ|白菜|きゅうり|胡瓜|もやし|よやし|わかめ|若布|ひじき|しめじ|えのき|しいたけ|椎茸|まいたけ|舞茸|エリンギ|きのこ|豚ひき肉|豚挽き肉|豚肉|豚もも肉|豚肉もも|鶏もも肉|鶏モモ|鶏肉|牛肉|ミンチ|豆腐|木綿豆腐|絹豆腐|油揚げ|卵|玉子|牛乳|ミルク|食パン|パン|ジャム|ヨーグルト|チーズ|米粉|小麦粉|せんべい|ブロッコリー|クリームコーン|ホットケーキミックス|ホットケーキMIX|ホンド鐘|てミンタス|無塩バター|無塩バタ|マーマレード|ねぎ|コーン缶|コーン笛|ハム|はるさめ|春雨|豆乳|オレンジ濃縮果汁|オレンジ果汁|粉かんてん|寒天|ツナ|ツナ缶|ツナ油漬|ツナ油漬け缶|缶詰|鮭|さけ|さば|鯖|白身魚|ちくわ|ハム|ベーコン|コーン|カレーフレーク|SBカレーフレーク|S&Bカレーフレーク|だいこん|大根|パイン缶|パイナップル缶|パイン|スパゲティ|スパゲッティ|パスタ|パイシート|バイシート|バナナ|りんご|みかん|いちご")
...
CANONICAL_INGREDIENT_PATTERNS = [
    ...
    ("牛乳", re.compile(r"(?<!和)牛乳|ミルク|(?:^|[^A-Za-z])Fh(?:$|[^A-Za-z])")),
    ...
    ("たまねぎ", re.compile(r"たまねぎ|玉ねぎ|玉葱|たまねを|たまねきぎ|療半と")),
    ("鶏もも(皮なし)", re.compile(r"鶏\s*(?:もも|モモ)\s*(?:肉)?(?:[（(]?皮ない[）)]?)?|鶏肉|とり\s*もも\s*肉")),
    ("もやし", re.compile(r"もやし|よやし|(?:^|[^ぁ-んァ-ン一-龥])もや(?:$|[^ぁ-んァ-ン一-龥])")),
    ("きゅうり", re.compile(r"きゅうり|きゆうり|さゅうり|きめゅうり|さきゅうり|きゅうの|胡瓜")),
    ...
    ("豚肉(もも)", re.compile(r"豚\s*(?:肉)?\s*もも\s*(?:肉)?|豚肉[（(]もも[）)]|豚肉")),
    ("ブロッコリー", re.compile(r"ブロッ?コリ[ー－]?|プロッ?コリ[ー－]?|ブロツコリ[ー－]?|ブロシンコリ|ブロシッコリー")),
    ("クリームコーン缶", re.compile(r"クリーム\s*コーン\s*(?:缶|かん|館)?")),
    ("コーン缶", re.compile(r"(?<!クリーム)コーン缶|コーン笛|コーン向|とうもろこし缶|トウモロコシ缶")),
    ("ホットケーキミックス", re.compile(r"ホット\s*ケーキ\s*(?:ミックス|MIX|Mix)|ホンド鐘|てミンタス", re.IGNORECASE)),
    ("無塩バター", re.compile(r"無塩\s*バタ(?:ー)?")),
    ("マーマレード", re.compile(r"マ[ーァ]?マレード|ママレード")),
    ("ねぎ", re.compile(r"ねぎ|ネギ|葱")),
    ("ハム", re.compile(r"ハム|ロースハム")),
    ("はるさめ", re.compile(r"はるさめ|春雨")),
    ("豆乳", re.compile(r"豆乳|とうにゅう|トウニュウ|豆[孔礼]")),
    ("オレンジ濃縮果汁", re.compile(r"オレンジ\s*(?:濃縮)?\s*果汁|濃縮\s*オレンジ\s*果汁|オレンジのうしゅく果汁")),
    ("粉かんてん", re.compile(r"粉\s*(?:かんてん|寒天|かんでん)|かんてん|寒天")),
    ("みかん缶", re.compile(r"みかん\s*(?:缶|かん|和缶|人缶)|ミカン缶|蜜柑缶")),
    ("SBカレーフレーク", re.compile(r"S\s*&\s*B\s*カレー\s*フレーク|Ｓ\s*＆\s*Ｂ\s*カレー\s*フレーク|SB\s*カレー\s*フレーク|カレー\s*フレーク|SBカレー", re.IGNORECASE)),
    ("だいこん", re.compile(r"だいこん|大根|ダイコン|だいこ|たいこん")),
    ("ツナ油漬け缶", re.compile(r"ツナ\s*(?:油漬け?|油づけ|缶)?|ツナ油漬け缶")),
    ("パイン缶", re.compile(r"パイン\s*(?:缶|かん)?|パイナップル\s*(?:缶)?")),
    ("スパゲティ", re.compile(r"スパゲッ?ティ|スパゲテイ|パスタ")),
    ("パイシート(冷凍)", re.compile(r"(?:冷凍)?\s*[パバ]イシート|[パバ]イシート\s*[（(]冷凍[）)]")),
...
def build_order_rows(source_rows: list[IngredientRow]) -> list[IngredientRow]:
    ...
    for (name, unit), info in aggregate.items():
        ...
        if name == "にんじん":
            formatted_quantity, formatted_unit = (str(int(round(quantity))) if abs(quantity - round(quantity)) < 0.000001 else f"{quantity:.3f}".rstrip("0").rstrip("."), "g")
        else:
